...
const responseFor = (body: BodyInit | null, contentType = "image/png", status = 200): Response =>
...
describe("ImageService", () => {
  it("stores fetched images under a hashed key", async () => {
    const { service, blobs } = serviceWith(responseFor(new Uint8Array([1, 2, 3])));
    const result = await service.fetchAndStore("u1", "https://a.com/og.png");
  ...
  it("rejects non-image responses", async () => {
    const { service } = serviceWith(responseFor("<html></html>", "text/html"));
    await expect(service.fetchAndStore("u1", "https://a.com")).rejects.toThrow(
  ...
  it("rejects failed fetches", async () => {
    ...
    await expect(service.fetchAndStore("u1", "https://a.com/missing.png")).rejects.toThrow(
      "status 404",
    );
  });
  ...
  it("rejects images over 5MB", async () => {
    ...
    await expect(service.fetchAndStore("u1", "https://a.com/big.png")).rejects.toThrow(
      "larger than 5MB",
    );
  });
});
