...
describe("image signing", () => {
  it("verifies a freshly signed path", async () => {
    ...
    await expect(
      verifyImagePath(SECRET, "u1", "abc", exp, signature, NOW),
    ).resolves.toBe(true);
  });
  ...
  it("rejects wrong keys, signatures and secrets", async () => {
    ...
    const signature = await signImagePath(SECRET, "u1", "abc", exp);
    await expect(
      verifyImagePath(SECRET, "u2", "abc", exp, signature, NOW),
    ).resolves.toBe(false);
    await expect(
      verifyImagePath(SECRET, "u1", "abc", exp, "0".repeat(64), NOW),
    ).resolves.toBe(false);
    await expect(
      verifyImagePath("other-secret", "u1", "abc", exp, signature, NOW),
    ).resolves.toBe(false);
  });
});
